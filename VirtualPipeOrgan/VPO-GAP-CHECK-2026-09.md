...
| 1 | **Web-/LAN-afstandsbediening** (telefoon/tablet als registers, setzer, pistons) | GO: open PR #2622 (12 sept 2026); Rusty Pipes (web-UI + REST, apr 2026); Aristide (headless daemon + console-client over HTTP); RPi-projecten | **Gedaan in 0.7.38** (opt-in, token, ingebedde pagina op 0.0.0.0:8766: registers, koppels, tremulant, setzer, volume; test-API op 127.0.0.1:8765 ongewijzigd) | Hoog — tablet op de speeltafel; sluit aan bij de trend "headless + tablet" | Middel: opt-in instelling "Afstandsbediening in het netwerk" (runtime aan/uit, LAN-bind, eenvoudig token/QR), één ingebedde HTML-pagina die de bestaande endpoints gebruikt |
